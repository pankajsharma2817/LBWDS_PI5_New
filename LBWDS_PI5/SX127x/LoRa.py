...
#   [FIX 2] spi moved from class variable to instance variable in __init__()
#            (was opened at import time before BOARD.setup(); also shared
#             across all instances which is unsafe)
#   No other changes needed in this file — all other bugs are in
#   board_config_pi5.py and node1_lora.py.
...
from .board_config_pi5 import BOARD
...
# Definition of the LoRa cla

class LoRa(object):
    # [FIX 2] spi was a class-level variable: `spi = BOARD.SpiDev()`
    # That caused two problems:
    #   a) SpiDev() is called at *import time*, before BOARD.setup() has
    #      opened the lgpio chip handle — SPI port is opened on an
    #      uninitialised board.
    #   b) All instances share the same handle — unsafe if two LoRa
    #      objects ever exist in the same process.
    # Fix: declare spi = None here; assign per-instance inside __init__()
    # only after the caller has run BOARD.setup().

    spi = None                        # [FIX 2] assigned in __init__
    ...
    def __init__(
        ...
        """
        # [FIX 2] Open SPI per instance, after BOARD.setup() has run
    ...
    def rx_is_good(self):
        ...
        return not any([flags[s] for s in ['valid_header',
    ...
    def set_freq(self, f):
        ...
        if self.mode not in (MODE.SLEEP, MODE.STDBY, MODE.FSK_STDBY):
        	sys.stderr.write(
            f"Warning: set_freq called in unexpected mode {
                self.mode}\n")
    ...
    def set_lna(self, lna_gain=None, lna_boost_lf=None, lna_boost_hf=None):
        ...
        if lna_gain is not None:
            # agc_auto_on must track lna_gain: GAIN=NOT_USED -> agc_auto=ON,
            # otherwise =OFF
            self.set_agc_auto_on(lna_gain == GAIN.NOT_USED)
        return retval
    ...
    def set_irq_flags_mask(
        ...
        for i, s in enumerate(['cad_detected', 'fhss_change_ch', 'cad_done',
                              'tx_done', 'valid_header', 'crc_error', 'rx_done', 'rx_timeout']):
            this_bit = locals()[s]
    ...
    def get_pkt_rssi_value(self):
        ...
        # See datasheet 5.5.5. p. 87
        return v - (164 if BOARD.low_band else 157)
    ...
    def set_modem_config_1(
            self,
            bw=None,
            coding_rate=None,
            implicit_header_mode=None):
        loc = locals()
        current = self.get_modem_config_1()
        loc = {s: current[s] if loc[s] is None else loc[s] for s in loc}
        val = loc['implicit_header_mode'] | (
            loc['coding_rate'] << 1) | (
            loc['bw'] << 4)
    ...
    def set_modem_config_2(
            self,
            spreading_factor=None,
            tx_cont_mode=None,
            rx_crc=None):
        loc = locals()
        # RegModemConfig2 contains the SymbTimout MSB bits. We tack the back on
        ...
        val = (
            ...
            loc['tx_cont_mode'] << 3) | (
            loc['rx_crc'] << 2) | current['symb_timout_lsb']
    ...
    def get_symb_timeout(self):
        ...
        msb, lsb = self.spi.xfer([SYMB_TIMEOUT_MSB, 0, 0])[1:]
        msb = msb & 0b11
        return lsb + 256 * msb

    def set_symb_timeout(self, timeout):
        bkup_reg_modem_config_2 = self.spi.xfer(
            [REG.LORA.MODEM_CONFIG_2, 0])[1]
        msb = timeout >> 8 & 0b11    # bits 8-9
        lsb = timeout - 256 * msb    # bits 0-7
        # bits 2-7 of bkup_reg_modem_config_2 ORed with the two msb bits
        reg_modem_config_2 = bkup_reg_modem_config_2 & 0xFC | msb
        old_msb = self.spi.xfer(
            [REG.LORA.MODEM_CONFIG_2 | 0x80, reg_modem_config_2])[1] & 0x03
    ...
    def set_preamble(self, preamble):
        ...
        old_msb, old_lsb = self.spi.xfer(
            [REG.LORA.PREAMBLE_MSB | 0x80, msb, lsb])[1:]
        return old_lsb + 256 * old_msb
    ...
    def get_dio_mapping_1(self, mapping):
        ...
        :rtype: list[int]
        """
        self.dio_mapping = [mapping >> 6 & 0x03,
                            mapping >> 4 & 0x03,
                            mapping >> 2 & 0x03,
                            mapping >> 0 & 0x03] + self.dio_mapping[4:6]
    ...
    def __str__(self):
        ...
        def onoff(i): return 'ON' if i else 'OFF'
        f = self.get_freq()
